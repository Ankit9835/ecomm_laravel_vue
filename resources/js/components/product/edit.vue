<template>
 ...
 <select class="form-control" id="exampleFormControlSelect1" v-model="form.sub_sub_category_id">
                        <option
                                                        v-for="data in subsubcategories"
                                                        :value="data.id"
                                                        >{{
            ...
            <div class="col-md-12">
             <input type="file" class="form-control" ref="file"  multiple  @change="uploadImage"/>
              <!-- <div  v-for="(image, key) in form.images" :key="key">
               ...
               <p style = "display:inline-block;" v-for="image in images"> <img v-bind:src="'http://127.0.0.1:8000/' + image.image" id = "em_photo" />  </p>
  ...
  methods:{
    ...
    productUpdate(){
     let id = this.$route.params.id
      axios.patch('/api/product/'+id,this.form,this.newimage)
     .then(() => {
      ...
      productImage(){
        let product_id = id;
         axios.get('/api/product/image/'+product_id)
       ...
       getCategory: function() {
        ...
        getSubCategory: function() {
            axios.get("/api/subcategory").then(
                function(response) {
